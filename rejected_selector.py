...
def checkModel(model, X, Y):
    print("checkModel:")
    scores = model.evaluate(x=X, y=Y, verbose=1)
    return scores[1] 
...
objects = minioClient.list_objects_v2(sys.argv[1], prefix='weights',
                              recursive=False)
...
for obj in objects:
    ...
    if os.path.isfile(filename):
        ...
        model_acc = checkModel(model,x_test,Y_test)
...
if (len(best_filename) > 0):
    ...
    shutil.copy2(best_filename,"weights-REJECTED_best-1-classifier-1.hdf5")

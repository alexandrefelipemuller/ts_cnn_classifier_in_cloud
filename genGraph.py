...
def getParent(object, minioClient):
    ...
    if not parent:
        ...
        m = re.search('weights-([0-9]*-[0-9]*)-best.hdf5', object.object_name)
        n = re.search('weights-([0-9]*-[0-9]*)-best.hdf5', parent)
...
for obj in objects:
    ...
    m = re.search('weights-([0-9]*-[0-9]*)-best.hdf5', filename)
    if m:
        n = re.search('([0-9]*)-([0-9]*)',m.group(1))
        roundname = n.group(1)
        nodename = n.group(2)
        nodes += "<node id=\""+nodeName2ID(m.group(1))+"\" positionX=\""+str(int(roundname)*200)+"\" positionY=\""+str(int(nodename)*800)+"\" mainText=\""+roundname+"-"+nodename+"\" upText=\"\"></node>\n"
